...
module Make (Toolbox : Iface.COMPILER_TOOLBOX) = struct
  ...
  struct
    ...
    let base_register2color_assignment : (reg, int) Hashtbl.t =
      Hashtbl.create 13
    ...
    let loop name f =
      let rec iter i =
        ...
        if should_restart then iter (succ i) else r
      in
      iter 0
    ...
    let compute_spill_costs infg =
      Logger.extra_debug (fun () -> logf "Computing dominators");
      let dom = Toolbox.DominatorsAnalysis.analyse cfg in
      Logger.extra_debug (fun () -> logf "Computing natural-loops");
      let nloops = Toolbox.NaturalLoopsAnalysis.analyse cfg dom in
      Logger.extra_debug (fun () -> logf "Computing spill-costs");
      let spill_costs = Toolbox.SpillCostsAnalysis.analyse cfg nloops in
      Logger.extra_debug (fun () -> Logger.dump_spill_costs spill_costs);
      spill_costs
    ...
    let simplify = failwith "not yet implemented"

    (* ------------------------------------------------------------------------
     *  Faza Select
     *)

    let select = failwith "not yet implemented"
    ...
    let build_coalescence () =
      ...
      let changed =
        measure "coalescence" (fun () ->
            Coalescencing.coalesce proc infg available_registers)
      in
      (infg, changed)

    let build_coalescence_loop () = loop "build-coalescence" build_coalescence
    ...
    let single_pass () =
      let init () =
        ...
        Hashtbl.replace_seq register2color_assignment
        @@ Hashtbl.to_seq base_register2color_assignment
      in
      Logger.extra_debug (fun () -> Logger.dump_ir_proc "begin-loop" proc);
      let init = measure "init" init in
      let infg = measure "build-coalescence " build_coalescence_loop in
      let spill_costs =
        measure "spillcosts" (fun () -> compute_spill_costs infg)
      in
